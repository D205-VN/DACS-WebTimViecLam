...
exports.completeCurrentInterview = async (req, res) => {
    ...
    try {
        ...
        const completedApplicationId = completed.rows[0]?.application_id || null;
        if (completedScheduleId) {
            const completedResult = await client.query(
                `SELECT
                 ...
                 LEFT JOIN applied_jobs aj ON aj.id = ms.application_id
                 LEFT JOIN users u ON u.id = COALESCE(ms.seeker_id, aj.user_id)
                 LEFT JOIN jobs j ON j.id = COALESCE(aj.job_id, $2)
                 WHERE ms.id = $1`,
                [completedScheduleId, room.job_id]
            );
            completedCandidate = completedResult.rows[0] || null;
        } else if (completedApplicationId) {
...
exports.saveHostInterviewEvaluation = async (req, res) => {
    try {
        ...
        const room = roomResult.rows[0];
        if (!room) {
            return res.status(404).json({ error: 'Không có quyền đánh giá từ phòng phỏng vấn này' });
        }

        const scheduleResult = await pool.query(
            `SELECT 1
             FROM meeting_schedules
             WHERE meeting_room_id = $1 AND application_id = $2
             LIMIT 1`,
            [room.id, applicationId]
        );

        if (!scheduleResult.rows.length) {
            return res.status(404).json({ error: 'Ứng viên không thuộc phòng phỏng vấn này' });
        }

        const saved = await talentInsightService.saveInterviewEvaluationForEmployer(
            room.employer_id,
            applicationId,
            req.body || {}
        );
        ...
        console.error('Save host interview evaluation error:', error);
        return res.status(error.statusCode || error.status || 500).json({
            error: error.statusCode || error.status ? error.message : 'Không thể lưu đánh giá phỏng vấn',
        });
    }
};
